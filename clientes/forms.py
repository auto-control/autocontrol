...
class clienteModelForm(forms.ModelForm):
	class Meta:
		...
		fields = '__all__'
		widgets = {
			'nombre': forms.TextInput(attrs={'required': True, 'class': 'form-control', 'maxlength': 35, 'pattern': '[A-Za-z ]+', 'title': 'Digite solo letras'}),
			'documento': forms.TextInput(attrs={'class': 'form-control', 'pattern': '[0-9]{1,12}', 'title': 'Digite solo numeros'}),
			'apellido': forms.TextInput(attrs={'required': True, 'class': 'form-control', 'maxlength': 35, 'pattern': '[A-Za-z ]+', 'title': 'Digite solo numeros'}),
